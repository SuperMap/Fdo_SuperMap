...
const UGGeoTransParams GeoTransParamsDefault;

class PROJECT_API UGPrjTranslator  
{
public:
	//! \brief 默认构造函数
	UGPrjTranslator();
	...
	//! \brief 得到当前转换器状态。1表示坐标系有效，0表示坐标系无效
...
//	UGbool Traverse_EqualArea_Cylindrical_Inverse(UGdouble& x, UGdouble& y);
	...
	//等积圆锥投影（当N_LATITUDE为2时称为亚尔勃斯投影(Alberts);
