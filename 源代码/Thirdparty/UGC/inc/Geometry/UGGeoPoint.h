 ...
 *  All Rights Reserved
 *  \version $Id: UGGeoPoint.h,v 1.37 2007/10/23 06:06:29 liwl Exp $
 */
...
public: // 得到和设置信息
	...
	//! \param pnt [in] 点坐标。
	//! \return 返回true。
	UGbool Make(const UGPoint2D& pnt);	

	//! \brief 构造点对象。
	//! \param dx [in] X值。
	//! \param dy [in] Y值。
	//! \return 返回true。
	UGbool Make(UGdouble dx, UGdouble dy);	
	
	//! \brief 拷贝构造点对象。
	//! \param geoPoint [in] 已有点对象。
	//! \return 返回true。
	UGbool Make(const UGGeoPoint& geoPoint);	
...
//	//! \brief 设置点坐标
	//! \param pnt 点坐标
	void SetPoint(const UGPoint2D& pnt);

	//! \brief 得到点坐标
	//! \return 返回点坐标
	const UGPoint2D& GetPoint() const;
