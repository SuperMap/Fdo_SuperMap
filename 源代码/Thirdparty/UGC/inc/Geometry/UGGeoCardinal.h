...
//! Cardinal曲线有一个非常优良的特性,就是控制点都是必然会通过的点,所以在编辑的时候感觉比较好
...
public:
	...
	//! \return 小于2个点,构造不成功,返回false。
    UGbool Make( UGPoint2D* pPoints, UGint nPointCount );

	//! \brief 通过复制构造Cardinal对象。
	//! \param geoCardinal [in] 原有的Cardinal。
	//! \return 原有Cardinal非法, 返回false。
