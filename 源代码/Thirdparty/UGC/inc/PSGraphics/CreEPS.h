...
class CAt
{
	public :
		...
		enum LINEDASH       { LD_KEEP = -1, SOLID, DOT, DASH, DOTDASH,NULLDASH };
		
		enum FILLINGPATTERN { FP_KEEP = -1, HEXDOT, CHECKERBOARD, STRIPE,
			CROSS,CROSS45,NULLBRUSH};
		...
		void  setLineThickness(float dThickness) {m_LineThickness = dThickness;}
		...
		//added by dongfei 2007-06-09
		/*! \brief  是否绘制文字轮廓 背景透明时才考虑
		*/
		void setHalo(bool bHalo) {m_bHalo = bHalo;};
		bool isHalo()  const {return m_bHalo;};		
		//added by dongfei 2007-06-12
		/*! \brief  设置字体 包括字体名和字大
		*/
		bool setFont(const CAt& A, FILE* file);
		/*! \brief 设置斜体
		*/
		bool isItalic() const {return m_bItalic;};
		void setItalic(bool bItalic = false) {m_bItalic = bItalic;};

		void setFontString(const char* strFontName) {strcpy(m_FontString,  strFontName);};
	protected :
...
// {{ added by fongfei 2007-05-31
class CAtCrossFilling : public CAt
{
	public :
		CAtCrossFilling( const float width, const float distance, const CAt& = CAt() );


		~CAtCrossFilling() {}
};

class CAtCross45Filling : public CAt
{
	public :
		CAtCross45Filling( const float width, const float distance, 
		                  const CAt& = CAt() );
		~CAtCross45Filling() {}
};

class CAtNullFilling : public CAt
{
		public :
		CAtNullFilling(  const CAt& = CAt() );
		~CAtNullFilling() {}
};
// }} added by fongfei 2007-05-31
/*********************************************************
...
class CreEPS
{
	...
	public :
		...
		void rectFill( const float x, const float y,
		               const float w, const float h, const CAt& attr = CAt() );
		void rectClip( const float x, const float y,
		...
		//added by dongfei 2007-05-20
		//! \brief 添加image到eps文件中 
		void AddImage(const float x,const float y,const int nWidth,const int nHeight,const float scaleX,const float scaleY,const int nBitsperPixel, const char* pBits);
		int  ASCII85Encode(const char *  pBits, unsigned int nCount);
		void OutByte(unsigned char c);
		void gsave();
		void grestore();
		void save();
		void restore();
		void fillTextBack(const float x, const float y,const char* str, const CAt& at);
		void fillTextRect(const float x, const float y,const float height,const char* str, const CAt& at);
		void strokeTextRect(const float x, const float y,const float height,const char* str, const CAt& at);
		void setFont(const CAt& attr);
		float getLineThickness()         const { return m_Attr.getLineThickness(); }
		void  setLineThickness(float dThickness) { m_Attr.setLineThickness(dThickness);}
	protected :
