...
#ifndef [!output UPPER_PREFIX]INSERTCOMMAND_H
#define [!output UPPER_PREFIX]INSERTCOMMAND_H

#ifdef _WIN32
#pragma once
#endif // _WIN32

class [!output PREFIX]InsertCommand :
    public FdoCommonFeatureCommand<FdoIInsert, [!output PREFIX]Connection>
{
...
private:
    [!output PREFIX]InsertCommand (FdoIConnection *connection);
    virtual ~[!output PREFIX]InsertCommand (void);

public:
    //
    // Prevent the use of the Assignment Operation by definning it and not implemeting it.
    // DO NOT IMPLEMENT
    [!output PREFIX]InsertCommand & operator= (const [!output PREFIX]InsertCommand &right);
...
#endif // [!output UPPER_PREFIX]INSERTCOMMAND_H
