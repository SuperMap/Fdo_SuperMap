// LangSetup.cpp: implementation of the CLangSetup class.
//
//////////////////////////////////////////////////////////////////////
...
CString GetResString(UINT uStringID, WORD wLanguageID)
{
	CString strRes;
	if (_hLangDLL)
		strRes.LoadString(_hLangDLL, uStringID, wLanguageID);
	if (strRes.IsEmpty())
		strRes.LoadString(GetModuleHandle(NULL), uStringID, LANGID_EN_US);
	return strRes;
}
...
CString GetResString(UINT uStringID)
{
	CString strRes;
	if (_hLangDLL)
		strRes.LoadString(_hLangDLL, uStringID);
	if (strRes.IsEmpty())
		strRes.LoadString(GetModuleHandle(NULL), uStringID);
	return strRes;
}
...
//	{LANGID_EL_GR,	_T("greek"),		FALSE,	_T("el_GR"),	1253,	_T("windows-1253")},	// Greek
	{LANGID_EN_US,	_T("english"),		FALSE,	_T("en_US"),	1252,	_T("windows-1252")},	// English
...
//	{LANGID_TR_TR,	_T("turkish"),		FALSE,	_T("tr_TR"),	1254,	_T("windows-1254")},	// Turkish
	{LANGID_ZH_CN,	_T("chs"),			TRUE,	_T("zh_CN"),	 936,	_T("gb2312")},			// Chinese (P.R.C.)
	...
	if (dwVerInfSize != 0)
	{
		...
		if (pucVerInf)
		{
			if (GetFileVersionInfo(const_cast<LPTSTR>((LPCTSTR)rstrLangDLL), 0, dwVerInfSize, pucVerInf))
			{
				...
				{
					bResult = (pFileInf->dwProductVersionMS == MAKELONG(VERSION_MIN, VERSION_MJR) &&
                               pFileInf->dwProductVersionLS == MAKELONG(VERSION_BUILD, VERSION_UPDATE));
				}
			}
			free(pucVerInf);
		}
...
{
	m_wLanguageID = LANGID_ZH_CN;	//lid
...
void CLangSetup::GetLanguages(CWordArray &aLanguageIDs)
{
...
void CLangSetup::SetLanguageID(WORD lid)
{
	m_wLanguageID = lid;
}
...
void CLangSetup::SetLanguage()
{
 	...
 	if (!bFoundLang)
	{
		...
		if (!bFoundLang)
		{
			LoadLangLib(GetLangDir(), LANGID_ZH_CN);
			m_wLanguageID = LANGID_ZH_CN;
			CString strLngChina = GetResString(IDS_MB_LANGUAGEINFO);
			AfxMessageBox(strLngChina, MB_ICONASTERISK);
		}
		else
			m_wLanguageID = lidLocale;
	}

	// if loading a string fails, set language to CN
...
bool CLangSetup::IsLanguageSupported(LANGID lidSelected, bool bUpdateBefore)
{
	InitLanguages(GetLangDir(), bUpdateBefore);
	if (lidSelected == LANGID_ZH_CN)
		return true;
	const SLanguage* pLang = _aLanguages;
	for (;pLang->lid;pLang++)
	{
...
CString CLangSetup::GetLangDLLNameByID(LANGID lidSelected)
{
	const SLanguage* pLang = _aLanguages;
	for (;pLang->lid;pLang++)
	{
...
CString CLangSetup::GetHtmlCharset()
{
...
CString & CLangSetup::GetLangDir()
{
	return m_strLangDir;
}

WORD CLangSetup::GetLanguageID()
{
